...
void init_mm(const uint32_t* kernel_end) {
  ...
  KERNEL_LOG_INFO("Kernel heap starts at 0x%x\n", memory_info.last_alloc);
}
...
void init_paging() {
  KERNEL_LOG_INFO("Initializing paging...\n");
  ...
  KERNEL_LOG_SUCCESS("Paging was successfully enabled!\n");
}
...
void* malloc(size_t size) {
  ...
  while ((uint32_t)mem < memory_info.last_alloc) {
    ...
    KERNEL_LOG_INFO("mem=0x%x a={.status=%d, .size=%d}\n", mem, a->status, a->size);
    ...
    if (a->size >= size) {
      a->status = 1;
      KERNEL_LOG_INFO("RE:Allocated %d bytes from 0x%x to 0x%x\n", size, mem + sizeof(alloc_t),
...
nalloc:;
  if (memory_info.last_alloc + size + sizeof(alloc_t) >= memory_info.heap_end) {
    KERNEL_LOG_ERROR("Cannot allocate bytes! Out of memory.\n");
    __asm__ volatile("cli; hlt"); // panic
  }
  ...
  KERNEL_LOG_INFO("Allocated %d bytes from 0x%x to 0x%x\n", size, (uint32_t)alloc + sizeof(alloc_t),
            memory_info.last_alloc);
